...
<div class="mb-6">
    ...
    <h2 class="text-xl font-bold text-slate-800 mt-1">Edit Role: {{ $role->name }}</h2>
    <p class="text-sm text-slate-500">{{ count($assignedPermissions) }} permission aktif</p>
</div>

<form method="POST" action="{{ route('panel.settings.roles.update', $role->id) }}" class="max-w-3xl">
    @csrf @method('PUT')
    ...
    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm mb-4">
        ...
        <input value="{{ $role->name }}" disabled class="w-full bg-slate-50 border-slate-300 rounded-xl text-sm px-4 py-2.5 text-slate-400">
        <p class="text-xs text-amber-600 mt-1">Role sistem tidak bisa di-rename.</p>
        @else
        <input name="name" value="{{ $role->name }}" required class="w-full border-slate-300 rounded-xl text-sm px-4 py-2.5">
        @endif
    </div>
        ...
        <div class="px-6 py-4 border-b border-slate-100 last:border-b-0">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2" id="group-{{ Str::slug($group) }}">
                ...
                <label class="flex items-center gap-2 text-xs text-slate-600 cursor-pointer hover:text-slate-800 py-1">
                    <input type="checkbox" name="permissions[]" value="{{ $perm }}" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" {{ in_array($perm, $assignedPermissions) ? 'checked' : '' }}>
                    <code class="text-[11px] bg-slate-50 px-1 py-0.5 rounded {{ in_array($perm, $assignedPermissions) ? 'bg-indigo-50 text-indigo-700' : '' }}">{{ $perm }}</code>
                </label>
                @endforeach
            </div>
        </div>
    ...
    <div class="mt-4 flex gap-3">
        <button class="bg-indigo-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-700">Update Role</button>
