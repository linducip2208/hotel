...
<div class="max-w-lg mx-auto">
    <div class="mb-6">
        <a href="{{ route('panel.hk.lost-found.show', $item->id) }}" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 mb-2 transition">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            Back to item
        </a>
        <h1 class="text-2xl font-bold text-gray-900">Edit Item</h1>
        <p class="text-sm text-gray-500 mt-1">Update lost & found item details</p>
    </div>
        ...
        <form method="POST" action="{{ route('panel.hk.lost-found.update', $item->id) }}" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="found_location" value="{{ old('found_location', $item->found_location) }}"
                           class="w-full text-sm border border-gray-200 rounded-xl px-4 py-2.5 bg-white text-gray-900 focus:ring-1 focus:ring-primary-500 focus:border-primary-500">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="found_date" value="{{ old('found_date', $item->found_date->toDateString()) }}" required
                               class="w-full text-sm border border-gray-200 rounded-xl px-4 py-2.5 bg-white text-gray-900 focus:ring-1 focus:ring-primary-500 focus:border-primary-500">
                    ...
                    <div>
                        ...
                        <select name="room_id" class="w-full text-sm border border-gray-200 rounded-xl px-4 py-2.5 bg-white text-gray-700 focus:ring-1 focus:ring-primary-500 focus:border-primary-500">
                            ...
                            @foreach ($rooms as $room)
                                <option value="{{ $room->id }}" @selected(old('room_id', $item->room_id) == $room->id)>Room {{ $room->number }} — {{ $room->roomType?->name }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    @if ($item->photo_path)
                    <div class="mb-2">
                        <img src="{{ Storage::url($item->photo_path) }}" class="w-24 h-24 rounded-lg object-cover border border-gray-200">
                    </div>
                    @endif
                    ...
                    <p class="text-xs text-gray-400 mt-1">Max 5MB. Leave empty to keep current photo.</p>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3"
                              class="w-full text-sm border border-gray-200 rounded-xl px-4 py-2.5 bg-white text-gray-900 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 resize-none">{{ old('notes', $item->notes) }}</textarea>
            ...
            <div class="flex items-center gap-3 mt-8 pt-6 border-t border-gray-100">
                ...
                <a href="{{ route('panel.hk.lost-found.show', $item->id) }}" class="flex-1 text-center bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm font-medium px-6 py-2.5 rounded-xl transition">
                    Cancel
                </a>
            </div>
        </form>
