...
class PegipegiAdapter extends BaseChannelAdapter
{
    protected function defaultBaseUrl(): string
    {
        return 'https://api.pegipegi.com/hotel/v2/';
    }
    ...
    protected function executeSync(string $operation, callable $fn): array
    {
        ...
        try {
            ...
            Log::channel('channel-manager')->error('Pegipegi sync failed', [
                'channel_id' => $this->channel->id,
                'operation' => $operation,
                'error' => $e->getMessage(),
            ...
            Log::channel('channel-manager')->error('Pegipegi connection error', [
                'channel_id' => $this->channel->id,
                'operation' => $operation,
                'error' => $e->getMessage(),
            ]);
            ...
            Log::channel('channel-manager')->error('Pegipegi sync unexpected error', [
                'channel_id' => $this->channel->id,
                'operation' => $operation,
                'error' => $e->getMessage(),
            ]);
